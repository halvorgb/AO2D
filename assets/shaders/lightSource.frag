...
void main(){
  ...
  vec3 ambianceOverride = ambiance + vec3(1.0, 1.0, 1.0);
  vec3 diffuseColor = color_override * texture2D(diffuse, f_UV).rgb;
  vec3 specularColor = vec3(0.0, 0.0, 0.0); // IGNORE SPECULAR!


  int skipLoop = nofLights - nofLights;     // IGNORE OTHER LIGHT SOURCES

  vec3 totalLighting = ambianceOverride * diffuseColor;
