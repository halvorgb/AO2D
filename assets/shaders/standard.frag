...
void main(){
  ...
  for (int i = 0; i < nofLights; ++i) {
    ...
    vec3 diffuseReflection  =
      diffuseColor * lightColors[i] * lightStrength * cosTheta /(distance_to_light*distance_to_light);
    ...
    vec3 specularReflection =
      specularColor * lightColors[i] * lightStrength * pow(cosAlpha, 5) / (distance_to_light*distance_to_light);
