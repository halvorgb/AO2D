...
-- Hide/show the cursor on left/right click
...
keyCallback inputStateIO window key _ action _
    ...
    | otherwise
        = do is <- readIORef inputStateIO
             let kb  = stillPressed key action $ isKeyboardInput is
                 kb' = setKeyInput kb key action
